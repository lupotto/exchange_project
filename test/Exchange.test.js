    ...
    describe('filling orders', async() => {
      ...
      describe('success', async() => {
        ...
        it('executes the trade & charge fees', async() => {
          ...
          balance.toString().should.equal(tokens(amount).toString(), 'user 1 recieved tokens')
          balance = await exchange.balanceOf(ETHER_ADDRESS, user2)
          balance.toString().should.equal(ether(amount).toString(), 'user 2 recieved ether')
          balance = await exchange.balanceOf(ETHER_ADDRESS, user1)
          balance.toString().should.equal(tokens('0').toString(), 'user 1 ether deducted')
          balance = await exchange.balanceOf(token.address, user2)
          balance.toString().should.equal(tokens(0.9).toString(), 'user 2 tokens deducted with fee aplied')
